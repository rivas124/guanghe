...
import { UpdatePwd } from "../../../../configs/componyApi/api";
...
export function Password() {
  ...
  const handleClick = () => {
    var data = {
      usercode: global.sessionStorage.getItem("username"),
      userpwd: userpwd,
      updatepwd: updatepwd,
    };
    UpdatePwd(data).then((res: any) => {
      if (res.data.msg === "The old password is incorrect") {
        setAlert(true);
      } else if (updatepwd !== userpassword) {
        setAlert1(true);
      } else if (
        res.data.msg !== "The old password is incorrect" &&
        updatepwd === userpassword
      ) {
        router.push("/login");
      }
    });
